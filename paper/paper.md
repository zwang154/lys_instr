...
Within this landscape, `lys_instr` addresses a specific need: reusing control, workflow, and GUI components across custom instrument configurations, while connecting acquired multidimensional data directly to the visualization and analysis tools in `lys`. It is closest to Qudi in separating hardware, logic, and GUI layers and overlaps with PyMoDAQ's actuator/detector dashboards and scan functionality. `lys_instr` organizes controller and multidimensional-detector interfaces, hardware-independent workflows, and GUI assembly under a consistent API. Once users implement hardware communication against these interfaces, they can reuse the workflow and GUI layers across instruments and focus on experiment-specific strategies and algorithms. Because this cross-layer API and integration with lys are package-level design choices, `lys_instr` was developed as a distinct package rather than as an extension of another control framework.

...
Many scientific experiments require measurements across wide parameter spaces to elucidate physical phenomena. As experiments involve longer acquisition times and more diverse instruments, efficient automation becomes increasingly valuable. Measurement workflows may also incorporate informatics-driven, condition-based optimization through external Python libraries, motivating reusable programmatic interfaces. `lys_instr` focuses specifically on software-coordinated control-and-detect workflows.

Building such systems repeatedly incurs costs at both the device and application levels. Hardware-specific communication must be combined with state monitoring and asynchronous execution, while scan logic, storage, visualization, and operator controls are often rebuilt for individual experiments. `lys_instr` addresses this overhead through common controller and multidimensional detector interfaces, together with reusable workflow and GUI components defined against those interfaces. GUIs complement programmatic control by providing real-time visualization, device-state monitoring, and manual intervention when human judgment is required during acquisition. The Python-based interfaces allow instrument control to be combined with lys and other Python libraries for scientific computing and informatics. Reusing these shared components allows users to concentrate their implementation effort on communication with specific instruments and procedures unique to each experiment.
...
`lys_instr` focuses on software-coordinated control-and-detect workflows. Its three-layer architecture organizes controller and detector abstractions, reusable workflow and GUI logic, and system assembly under a consistent API. Developing `lys_instr` as a distinct package allowed this API to span all three layers while supporting direct integration with the Python-based `lys` platform. This integration combines instrument control with live multidimensional visualization and analysis, including MultiCut, in the same environment. Users add hardware-specific communication and specialized procedures through custom components.

...
Modern scientific research often relies on systematic measurements to elucidate physical phenomena. Such experiments may span wide parameter spaces, require long acquisition times, or coordinate multiple instruments, making efficient automation increasingly valuable. Measurement workflows may also incorporate informatics-driven, condition-based optimization, further motivating reusable and interoperable programmatic interfaces.

However, building such a measurement system remains time-consuming for researchers. At the low level, instrument-control implementations must accommodate diverse communication protocols (*e.g.*, TCP/IP, VISA, and serial), often tying higher-level code to device-specific details and limiting device interchangeability and reuse across systems. 
At the high level, coordinating workflows that combine conditional logic, iterative processes, and advanced algorithms across multiple libraries frequently leads to redundant implementations, reducing development efficiency. For example, capturing temperature-dependent images with a camera and acquiring temperature-dependent spectra share the same general workflow: iterative temperature adjustment followed by data acquisition. Nevertheless, this logic is often implemented separately for different experiments.
Moreover, for long or complex measurements, graphical user interfaces (GUIs) provide real-time visualization, device-state monitoring, and support for manual intervention. Implementing responsive GUIs for these low- and high-level functions, however, requires familiarity with GUI frameworks and event-handling mechanisms. These requirements impose substantial development overhead and highlight the need for a control framework that balances architectural flexibility with reduced implementation complexity.


# State of the field

Scientific instrument-control software includes platforms with different priorities. LabVIEW [@LabVIEW] and MATLAB’s Instrument Control Toolbox [@MATLAB] integrate instrument communication, workflows, and GUI development. QCoDeS [@QCoDeS] and PyMeasure [@PyMeasure] provide instrument abstractions and experimental procedures. Atomize [@Atomize] and fsc2 [@fsc2] support script-based experiments through reusable device operations. Bluesky [@Bluesky] separates hardware abstraction from reusable orchestration plans and optional GUI clients, while PyMoDAQ [@PyMoDAQ] combines actuator and detector plugins with dashboards and scan extensions. Qudi [@Qudi] explicitly separates hardware, experiment logic, and GUI layers. Taken together, these systems show that hardware, workflow, and GUI abstractions are well established; the relevant differences lie in how they are integrated and which experimental workflows they target.

Among these systems, Qudi provides the closest point of comparison for `lys_instr`. Its mature modular architecture separates hardware, experiment logic, and GUI modules, while its logic layer supports application-specific data processing and analysis. A central design goal of `lys_instr`, however, is to support experiments that generate increasingly complex multidimensional datasets, for which immediate inspection, analysis, and visualization during acquisition are both challenging and valuable. Rather than reimplementing general multidimensional visualization and analysis capabilities within Qudi, `lys_instr` is designed to integrate directly with `lys`, a general-purpose platform that already provides them. This integration allows acquired data to be inspected, analyzed, and visualized in the same environment used for instrument control. In addition, `lys_instr` provides reusable controller and detector abstractions and GUI-configurable nested scan workflows, allowing measurement components to be reused across hardware configurations.


# Software design

The three layers and their relationships are summarized in \autoref{fig:fig1}. Each layer applies established object-oriented design patterns [@Gamma:1994], including *Template Method*, *Bridge*, *Composite*, *Observer*, and *Mediator*, to define how components are extended, composed, and connected.

...
Scientific instrument-control software spans environments with different priorities. LabVIEW [@LabVIEW] and MATLAB’s Instrument Control Toolbox [@MATLAB] integrate instrument communication, workflows, and GUI development. QCoDeS [@QCoDeS] and PyMeasure [@PyMeasure] provide instrument abstractions and experimental procedures. PyMoDAQ [@PyMoDAQ] combines actuator and detector plugins with dashboards and scan extensions, while Bluesky [@Bluesky] separates hardware abstraction from reusable orchestration plans and optional GUI clients. Qudi [@Qudi] explicitly separates hardware, experiment logic, and GUI layers. Atomize [@Atomize] and fsc2 [@fsc2] support script-based experiments through reusable device operations. Together, these systems provide abstractions at the hardware, workflow, and GUI levels, but integrate them differently and target different workflows.

Among these systems, Qudi and PyMoDAQ provide the closest points of comparison for `lys_instr`. Qudi uses a three-layer architecture that decouples hardware, logic, and GUI modules, supporting reuse across hardware implementations. While its GUIs configure specific measurement toolchains, workflow composition remains specific to each toolchain rather than being provided through a shared graphical interface for composing and reorganizing nested measurement operations. PyMoDAQ provides GUI-configurable multidimensional and sequential scans, but organizes its functionality around instrument plugins, control modules, and dashboard extensions rather than an explicit three-layer hierarchy. The distinction of `lys_instr` lies in combining these strengths: an explicit three-layer architecture and GUI-based composition of nested workflows. Its Scan GUI allows users to construct and reorganize nested scans that coordinate controllers, switches, and detector acquisition. Once users implement hardware communication against its controller and multidimensional-detector interfaces, the same workflow and GUI components can be reused across hardware configurations, while acquired data can be visualized and analyzed directly through `lys`. These package-level design choices motivated the development of `lys_instr` as a distinct package rather than as an extension of another control framework.

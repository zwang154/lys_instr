...
Among these systems, Qudi and PyMoDAQ are the closest comparisons to `lys_instr`. Qudi has a similar layered architecture, but its workflows and GUIs are generally implemented as application-specific modules rather than composed through a common scan GUI. PyMoDAQ provides device abstractions and GUI-based scan configuration, but organizes workflows around scanner types and extensions rather than an editable hierarchy of nested operations. `lys_instr` combines layered separation with a GUI for constructing and reorganizing nested scans that coordinate controllers, switches, and detector acquisition. Once users implement hardware communication against its controller and multidimensional-detector interfaces, the same workflow and GUI components can be reused across hardware configurations, while acquired data can be visualized and analyzed directly through `lys`. This combination shaped the package architecture, motivating the development of `lys_instr` as a distinct package rather than as an extension of another control framework.
